...
console.log(`The total number of commits equals ${commits}.`)
...
for (let i = 0; i < githubData.length; i++) {
    if (githubData[i].type == "DeleteEvent") {
        deleteEvent ++;
...
console.log(`DeleteEvents = ${deleteEvent} \nPushEvents = ${pushEvent} \nPullRequestEvents = ${pullRequestEvent} \nIssueCommentEvents = ${issueCommentEvent} \nCreateEvents = ${createEvent}.` );
...
for (let i = 0; i < githubData.length; i++) {
    if (githubData[i].type == "PullRequestEvent" && githubData[i].payload.action == "closed" && githubData[i].payload.pull_request.merged_by.login == "stevebrownlee") {
        const pullUser = githubData[i].payload.pull_request.user.login
        if (pullUsers.indexOf(pullUser) === -1 ) {
            pullUsers.push(pullUser) 
        }
    }
}
